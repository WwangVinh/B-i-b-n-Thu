...
SELECT last_name, first_name, address, start_date FROM employees;
-- 4.	Địa chỉ, điện thoại của nhà cung cấp có tên giao dịch VINAMILK
SELECT *FROM suppliers;
SELECT address, phone FROM suppliers WHERE trading_name = 'VINAMILK';
-- 5.	Mã và tên của các mặt hàng có giá trị lớn hơn 100000 và số lượng hiện có ít hơn 50
SELECT product_id, product_name FROM products WHERE price >'100000' AND quantity < '50';
-- 6.	Cho biết mỗi mặt hàng trong công ty do ai cung cấp
SELECT products.product_name, suppliers.company_name FROM products INNER JOIN suppliers ON suppliers.company_id=products.company_id;
-- 7.	Công ty Việt Tiến đã cung cấp những mặt hàng nào
SELECT products.product_name, suppliers.company_name FROM products INNER JOIN suppliers ON suppliers.company_id=products.product_id WHERE company_name='Công ty may mặc Việt Tiến' ;
...
FROM products 
INNER JOIN suppliers ON suppliers.company_id=products.company_id
...
SELECT DISTINCT customers.trading_name, products.product_name
FROM customers
...
SELECT *FROM orders;
SELECT employees.employee_id, employees.last_name, employees.first_name, orders.order_date, orders.shipping_address
...
SELECT employee_id, last_name, first_name, base_salary + allowance AS salary
FROM employees
-- 12.	Trong đơn đặt hàng số 3 đặt mua những mạt hàng nào và số tiền mà khách
-- hàng phải trả cho mỗi mặt hàng là bao nhiêu(số tiền phải trả = số lượng x giá bán – số lượng x giá bán x mức giảm giá / 100)
